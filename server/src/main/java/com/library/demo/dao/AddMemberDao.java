...
//					System.out.println(jsonArray);
		...
		try {
			...
			try (Connection conn = DriverManager.getConnection(url, user, pass);
				...
				String updateQuery = "UPDATE library_management.user_details SET "
						+ "fname ='"+result.getfName()+"',lname ='"+result.getlName()+"',age ="+result.getAge()+" ,email ='"+result.getEmail()+"',phnumber ="+result.getPhoneNumber()+",outstandingdebt ="+result.getOutstandingDebt()+" ,updated_date ='"+result.getUpdatedDate()+"' WHERE member_id ="+result.getMemberId()+" ;"
						+ ""
						+ ""
						+ "";
	...
	public Responses getoneMember(AddMemberModel result) {
		...
		try {
			Class.forName("com.mysql.cj.jdbc.Driver");
			String getoneQuery ="select * from  library_management.user_details where member_id =" +result.getMemberId() + ";";
			System.out.println(getoneQuery);
			try (Connection conn = DriverManager.getConnection(url, user, pass);
					Statement st = conn.createStatement();
